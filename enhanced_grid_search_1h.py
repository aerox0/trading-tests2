#!/usr/bin/env python3
"""
Enhanced Grid Search for 1H timeframe with expanded parameter space
...
def enhanced_grid_search(df: pd.DataFrame) -> Tuple[Dict, Dict]:
    """
    Enhanced grid search with expanded parameter space for 1H timeframe
    ...
    """
    print("\n" + "=" * 80)
    print("ENHANCED GRID SEARCH WITH EXPANDED PARAMETER SPACE (1H TIMEFRAME)")
    print("=" * 80)
    ...
    param_grid = {
        "ema_fast_period": [20, 30, 34, 40, 50, 55, 60],
        "ema_slow_period": [100, 120, 144, 168, 200, 250],
        "atr_period": [14],
        "atr_multiplier_sl": [0.4, 0.5, 0.6, 0.7],
        "atr_multiplier_tp": [1.5, 2.0, 2.5, 3.0, 3.5],
        "rsi_period": [7, 14, 21],
        "volume_multiplier": [0.8, 1.0, 1.2],
        "position_size_pct": [0.4, 0.5, 0.6, 0.7],
    }
            ...
            for atr_p in param_grid["atr_period"]:
                for atr_sl in param_grid["atr_multiplier_sl"]:
                    for atr_tp in param_grid["atr_multiplier_tp"]:
                        # Only test reasonable R:R ratios (TP should be >= SL)
                        if atr_tp < atr_sl:
                            continue
                                ...
                                for pos_size in param_grid["position_size_pct"]:
                                    ...
                                    if iteration % 100 == 0 or iteration <= 20:
                                        print(
                                            ...
                                            f"EMA({ema_fast},{ema_slow}) SL({atr_sl}) TP({atr_tp}) RSI({rsi_p}) Size({pos_size * 100:.0f}%) | "
                                    ...
                                    if combined_score > best_score:
                                        ...
                                        print(
                                            f"  *** NEW BEST *** Score: {combined_score:.4f}"
                                        )
    ...
    for i, result in enumerate(all_results[:10], 1):
        ...
        print(
            f"   Train: Return {result['train_return']:+.2f}%, Sharpe {result['train_sharpe']:.2f}, Trades {result.get('train_trades', 0)}"
        )
        print(
            f"   Test:  Return {result['test_return']:+.2f}%, Sharpe {result['test_sharpe']:.2f}, Trades {result.get('test_trades', 0)}"
        )
...
def main():
    """Run enhanced grid search for 1H"""
    ...
    best_config, results = enhanced_grid_search(df)
