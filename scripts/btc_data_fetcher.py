#!/usr/bin/env python3
"""
Data fetcher for BTC/USDT historical data for Nautilus backtesting
"""
...
def fetch_btc_data_4h(symbol="BTC/USDT", period_days=730):
    """
    Fetch BTC/USDT 4H historical data from Binance
    ...
    while current_time < end_time:
        ...
        ohlcv = exchange.fetch_ohlcv(
            symbol, timeframe="4h", since=current_time, limit=limit
        )
    ...
    print(f"Fetched {len(df)} 4H candles for {symbol}")
...
def convert_to_nautilus_format(
    df: pd.DataFrame, output_file: str = "btcusdt_4h.csv"
) -> Path:
    """
    Convert dataframe to Nautilus format and save to CSV
